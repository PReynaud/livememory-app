...
- Hosted schema is pushed by `.github/workflows/deploy-migrations.yml` on `main` (`supabase db push --db-url`). It needs repo secret `SUPABASE_DB_URL` (direct Postgres URI). CI `supabase db reset` only refreshes the GitHub Actions local database.

...
- `pnpm db:reset` applies every migration on an empty public schema, then seed. Hosted databases already have rows. CI runs `pnpm db:check-latest-migration` after `supabase start` so the latest file is applied on representative Event/Concert rows (catches SQLSTATE 55006 and similar).
...
- **Bring the stack up:** with the Docker daemon running, `pnpm db:start` then `pnpm db:reset` (applies `supabase/migrations` + `supabase/seed.sql`). After changing schema, run `pnpm db:types` and commit `app/types/database.types.ts` (CI fails on any diff — see Known pitfalls). CI also runs `pnpm db:check-latest-migration` so the newest SQL file is applied on existing rows, not only on an empty database.

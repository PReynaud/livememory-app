...
One artist or group performing on a date, with an optional time. Belongs to exactly one Event. Optional Stage/Scene. Attendance is per User. Notes are Event-owner-only in v1.
...
Where an Event or Concert happened (venue, city, festival site, or combination). Inherited from the Event unless that Event allows overrides.

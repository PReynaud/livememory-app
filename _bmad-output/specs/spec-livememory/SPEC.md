...
- Place is the city. Stage/Scene is the venue or stage name. An Event with no Stage/Scene list allows Concerts without a Stage/Scene. A defined list is an open suggestion set: the User may pick an existing name or type a new one, which is added to the Event. Stage/Scene stays optional.
...
- Concert create identity is the Event owner's journal. Artist and Stage/Scene name matches are case-insensitive. Same owner + artist + date + clock time + Stage/Scene (or both empty): do not insert; attach to the existing Concert (do not move it). Event is not part of the key: a timed match on another owned Event still attaches without reparenting. A timed match at a different Stage/Scene creates a second Concert. Same identity at a different effective Place (city): refuse. Same owner + artist + date + Stage/Scene with time missing on one or both: ask attach (may then set time) or create a second Concert; UI and MCP must offer the same choice. Same artist + date + different times: allowed without asking. Moving a Concert between Events is a separate owner operation, not create/attach.
- Add Concert from nav always chooses an Event first (existing, New night, or New festival). Artist, Place (city), and Stage/Scene suggest from a shared **name** catalog after three characters (not a canonical artist database). Required fields are marked. Date and Time sit on one row. After the first Artist is filled, Add another artist adds extra artist rows; one Save creates those Concerts.

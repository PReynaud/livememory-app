...
- **Rule:** Every Concert belongs to exactly one Event (`concerts.event_id` not null, no junction). Scope of identity is the Event **owner's** journal, not all Users. Artist and Stage/Scene name matches are case-insensitive. Domain create returns exactly one of: `created`, `attached`, `needs_choice`, `impossible_place`. (1) same owner, artist, date, clock time, and Stage/Scene (or both empty) → `attached`: do not insert and do not change `event_id`; return the existing Concert (caller navigates there). Event is **not** part of this key — a timed match on another owned Event still attaches. A timed match at a **different** Stage/Scene → `created`. (2) same owner, artist, date, time, Stage/Scene, different effective place (city) → `impossible_place`: refuse. (3) same owner, artist, date, Stage/Scene, time missing on one or both sides → `needs_choice`; UI and MCP must ask: attach (may then set time on the existing row) or create a second Concert. (4) same artist, same date, different times → `created` allowed. Move between Events is a **separate** owner operation, not create/attach. Database unique-guards timed exact matches including Stage/Scene name; the untimed case cannot be only a unique index. A shared `name_catalog` table stores artist, city, and venue/stage **strings** for autocomplete only.
...
- Realtime fan-out, username rename, Event-link rotation, owner kick, joiner roster, joiner notes, concert duration / overlap windows beyond AD-10 (same artist + date + different times remain allowed; Stage/Scene is not part of identity), import from old LiveMemory, OAuth for MCP, PWA.
